...
export default function CheckoutForm() {
  ...
  const grandTotal = useMemo(() => {
    // reduce -> menjumlahkan semua total harga per produk:
    // curr.price * curr.quantity -> total harga produk saat ini.
    // prev -> akumulasi jumlah sebelumnya.
    // 0 -> nilai awal akumulasi.
        ...
        <div className="w-full bg-white border border-[#E5E5E5] flex flex-col gap-[30px] p-[30px] rounded-3xl">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <p className="font-semibold">{rupiahFormat(0)}</p>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="flex shrink-0">
...
/*
    Note:
    useMemo adalah React hook yang menyimpan hasil perhitungan (memoization).
    Jadi, fungsi di dalam useMemo hanya akan dihitung ulang kalau dependencies berubah — dalam hal ini [products].

    Tanpa useMemo, setiap kali komponen render ulang, reduce akan dieksekusi meskipun products tidak berubah.
    Kalau datanya banyak atau hitungan kompleks, ini bisa bikin aplikasi jadi lambat.

    Dengan useMemo:
    Kalau products tidak berubah, React langsung pakai hasil sebelumnya (grandTotal lama).
    Kalau products berubah, barulah React jalankan ulang reduce.
*/
